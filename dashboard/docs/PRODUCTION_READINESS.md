...
| Hidden operational IDs | Cron, Kanban run, worker, and event identifiers now use public refs by default. | `npm run verify` and `npm run test:security` fail on raw worker/current-run/run ID keys in hidden-label payloads. |
